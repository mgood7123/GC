...
typedef struct {
	...
	void (*callback_source)(void*);
	void (*callback_error)(void*);
	void (*callback_forgot_to_init)(void*);
	void (*callback_unfreed_info)(void*);
	void (*callback_unfreed_total)(void*);
	void (*callback_add)(void*);
	void (*callback_remove)(void*);
	void (*callback_realloc)(void*);
...
#define Garbage_Collector_Cleanup() tgc_run(&Garbage_Collector_Program, __FILE__, __LINE__, __FUNCTION__)
...
void Garbage_Collector_Initializer = Garbage_Collector_Start();
...
#define Garbage_Collection_Pointer_Add_First_Pointer 1
#define Garbage_Collection_Pointer_Add_Already_Exists 2
#define Garbage_Collection_Pointer_Add_New_Pointer 3
...
void * tgc_callbacks_default_add(char * pointer, int status) {
	if (status == Garbage_Collection_Pointer_Add_First_Pointer || status == Garbage_Collection_Pointer_Add_New_Pointer) GCDEBUG_LIST printf("Garbage Collector: adding %p to the list\n", pointer);
	else if (status == Garbage_Collection_Pointer_Add_Already_Exists) GCDEBUG_LIST printf("Garbage Collector: %p has already been added the list\n", pointer);
	return NULL;
}

void * tgc_callbacks_default_remove(char * pointer) {
	GCDEBUG_LIST printf("Garbage Collector: removing %p from the list\n", pointer);
	return NULL;
}

void * tgc_callbacks_default_realloc(char * pointer, size_t old, size_t new) {
	GCDEBUG_REALLOC printf("Garbage Collector: reallocating %p from %zu to %zu\n", pointer, old, new);
	return NULL;
}

void * tgc_callbacks_default_free(char * pointer) {
	GCDEBUG_FREE printf("Garbage Collector: freeing %p\n", pointer);
	return NULL;
}

void * tgc_callbacks_default_exit(char * function) {
...
static void tgc_add_ptr(
  ...
  while (1) {
    ...
    if (h == 0) {
    	gc->items[i] = item;
  		if (gc->callbacks.callback_add) gc->callbacks.callback_add(ptr, Garbage_Collection_Pointer_Add_First_Pointer);
    	if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
    	return;
    }
    if (gc->items[i].ptr == item.ptr) {
  		if (gc->callbacks.callback_add) gc->callbacks.callback_add(ptr, Garbage_Collection_Pointer_Add_Already_Exists);
    	if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
    	return;
    }
    p = tgc_probe(gc, i, h);
    if (j >= p) {
  		if (gc->callbacks.callback_add) gc->callbacks.callback_add(ptr, Garbage_Collection_Pointer_Add_New_Pointer);
      tmp = gc->items[i];
      gc->items[i] = item;
      item = tmp;
      j = p;
...
void tgc_start(tgc_t *gc) {
  if (gc->callbacks.callback_entry) gc->callbacks.callback_entry(__FUNCTION__);
  puts("Starting Garbage Collector");
  int a = 0;
  gc->init = 1;
  ...
  gc->loadfactor = 0.9;
  gc->sweepfactor = 0.5;
  puts("Garbage Collector successfully started");
  if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
}

void tgc_stop(tgc_t *gc, char * src_file, int src_line, char * src_func) {
  if (gc->callbacks.callback_entry) gc->callbacks.callback_entry(__FUNCTION__);
  puts("Stopping Garbage Collector");
  if (gc->callbacks.callback_source) gc->callbacks.callback_source(src_file, src_line, src_func);
  tgc_sweep(gc, src_file, src_line, src_func);
  free(gc->items);
  free(gc->frees);
  gc->init = 0;
  puts("Garbage Collector successfully stopped");
  if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
}
...
void tgc_resume(tgc_t *gc) {
  if (gc->callbacks.callback_entry) gc->callbacks.callback_entry(__FUNCTION__);
  gc->paused = 0;
  puts("Garbage Collector Resumed");
  if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
}
...
size_t tgc_get_size(tgc_t *gc, void *ptr, char * src_file, int src_line, char * src_func) {
  if (gc->callbacks.callback_entry) gc->callbacks.callback_entry(__FUNCTION__);
  if (gc->callbacks.callback_source) gc->callbacks.callback_source(src_file, src_line, src_func);
  tgc_ptr_t *p  = tgc_get_ptr(gc, ptr, src_file, src_line, src_func, src_func, "size");
  if (p) { 
  	if (gc->callbacks.callback_exit) gc->callbacks.callback_exit(__FUNCTION__);
  	return p->size; 
  }
